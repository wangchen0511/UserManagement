...
import com.user.management.util.DateUtil;
...
public class RequestSigningAuthorizationService implements AuthorizationService {
                ...
                .build(
                        new CacheLoader<String, Nonce>() {
                            public Nonce load(String key) throws Exception {
                                return new Nonce(ZonedDateTime.now(), key);
                            }
                        });
     ...
     */
    public ExternalUser authorize(AuthorizationRequestContext context) throws NoSuchAlgorithmException, UnsupportedEncodingException {
     ...
     * @throws NoSuchAlgorithmException 
     */
    private boolean isAuthorized(User user, AuthorizationRequestContext authorizationRequest, String hashedToken) throws NoSuchAlgorithmException, UnsupportedEncodingException {
        Assert.notNull(user);
     ...
     * @throws UnsupportedEncodingException 
     */
    private String encodeAuthToken(String token, String unencodedRequest) throws NoSuchAlgorithmException, UnsupportedEncodingException {
        String strBeforeHash = token + ":" + unencodedRequest;
        
        MessageDigest digest = MessageDigest.getInstance("SHA-256");
        byte[] hash = digest.digest(strBeforeHash.getBytes("UTF-8"));
    ...
    private void validateRequestDate(String requestDateString) {
        ZonedDateTime date = DateUtil.getDateFromIso8061DateString(requestDateString);
        ZonedDateTime now = ZonedDateTime.now();
        if (!(date.isAfter(now.minusMinutes(config.getSessionDateOffsetInMinutes())) &&
                        date.isBefore(now.plusMinutes(config.getSessionDateOffsetInMinutes())))) {
    ...
    private void validateNonce(String nonceValue) {
        ...
        Duration tolerance = Duration.between(nonce.timestamp, ZonedDateTime.now());
        if (tolerance.compareTo(Duration.ofMillis(NONCE_CHECK_TOLERANCE_IN_MILLIS)) >= 0) {
    ...
    private static class Nonce {
        ...
        Nonce(ZonedDateTime time, String nonce) {
            this.timestamp = time;
